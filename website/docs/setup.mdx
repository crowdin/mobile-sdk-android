---
description: Configure the Crowdin Android SDK in your app - wrap the context, enable Over-The-Air Content Delivery, and explore config options and tips.
---
...
Inject Crowdin translations into the Context by adding the *override* method to your *BaseActivity* class. If you have already migrated to [AppCompat](https://developer.android.com/jetpack/androidx/releases/appcompat) **1.2.0+** version, use this method:
...
| `withUpdateInterval`   | Translations update interval in seconds. The minimum and default value is 15 minutes (900 seconds). Visit the [Cache](/cache) page for more details | `withUpdateInterval(900)`                                                                                       |
...
**2.** In case you have custom views that use `TypedArray` and `styleable` attributes, you will need to use the following approach:
...
It has a callback method `onDataChanged()` that can be used to invalidate your UI (TextView, Menu, etc.). It will use downloaded resources automatically.
...
**6.** In case you have a custom `TextView` subclass with a string specified in XML, make sure its class name contains `TextView` (e.g., `PlaceholderTextView`). Otherwise, the SDK will skip this view during the inflating process and it won't be translated.

## Limitations

1. Plurals are supported from Android SDK version 24.
2. `TabItem` text added via XML won't be updated. There is a workaround: you can store `TabItem` titles in a string array and add tabs dynamically.
3. `PreferenceScreen` defined via XML is not supported.
